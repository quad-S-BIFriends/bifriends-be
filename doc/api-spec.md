...
자녀 모드·부모 모드 공통. JWT는 Stateless이므로 서버에서 토큰을 무효화하지 않습니다. 클라이언트가 저장된 `accessToken`·`refreshToken`을 폐기합니다.
...
**PATCH** `/api/v1/members/me/settings` · **JWT 필수**

이름·학년·관심사를 한 번에 저장합니다. `null`인 필드는 변경하지 않습니다.

**Request Body** (모두 선택)

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `nickname` | string | X | 아동 이름 (1~20자) — null이면 변경 안 함 |
| `grade` | int | X | 학년 (3~6) — null이면 변경 안 함 |
| `interests` | array\<Interest\> | X | 관심사 목록 (최대 3개) — null이면 변경 안 함, 빈 배열이면 전체 삭제 |

```json
{ "nickname": "민준이", "grade": 4, "interests": ["ANIMAL", "SPACE"] }
```
...
**DELETE** `/api/v1/members/me` · **JWT 필수** · **부모 모드 전용**

부모 PIN 확인 후 회원 및 모든 관련 데이터를 삭제합니다. PIN 불일치 시 `403` 반환.

**Request Body**

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `parentPassword` | string | O | 부모 모드 PIN |

```json
{ "parentPassword": "1234" }
```

**Response** `204 No Content` — 탈퇴 완료

**Response** `403 Forbidden` — 부모 PIN 불일치
...
**주간 안전 보고서** (`POST /api/v1/ai/batch/weekly-safety`):
```json
{ "member_id": 1, "week_start": "2026-05-26", "week_end": "2026-06-01" }
```

**주간 성장 리포트** (`POST /api/v1/ai/report/weekly`):
```json
{ "member_id": 1, "week_start": "2026-05-26", "week_end": "2026-06-01", "grade": 4 }
```
